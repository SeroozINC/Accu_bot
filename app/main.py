...
# Prosty cache w pamięci (żeby nie zjadać limitów)
_prices_cache = {"ts": 0.0, "data": None}
PRICES_TTL_SECONDS = 1000
...
def api_prices(request: Request):
    # (opcjonalnie) jeśli chcesz, żeby tylko zalogowani mogli pobierać:
    # r = require_login(request)
    # if r: return {"error": "not authorized"}
    ...
    if not CMC_API_KEY:
        ...
        data = {"btc_usd": None, "eth_usd": None, "source": "coinmarketcap", "error": "missing_api_key"}
    ...
    try:
        ...
        btc = data_node.get("BTC", {}).get("quote", {}).get(CMC_CONVERT, {}).get("price")
        eth = data_node.get("ETH", {}).get("quote", {}).get(CMC_CONVERT, {}).get("price")

        result = {
            "btc_usd": float(btc) if btc is not None else None,
            "eth_usd": float(eth) if eth is not None else None,
    ...
    # fallback: jeśli CMC padnie, zwróć ostatnie cache jeśli istnieje, inaczej null
    if _prices_cache["data"]:
        return _prices_cache["data"]

    return {"btc_usd": None, "eth_usd": None, "source": "coinmarketcap", "error": "fetch_failed"}
